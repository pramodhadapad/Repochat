...
class ChatService {
  ...
  buildPrompt(intent, question, context, history) {
    ...
    if (NON_CONTEXT_INTENTS.has(intent)) {
      return `You are RepoChat, a friendly and helpful AI programming assistant.
${historyBlock}User: "${question}"
Reply conversationally in 1-2 sentences. Be warm and natural. Do not act like a robot.
Answer:`;
    }
  ...
  async chat(question, repoId, user, history = '') {
    ...
    if (!finalContext.trim()) {
      try {
        ...
        const chunks = await VectorIndexer.search(repoId, queryVector, 5, trimmedQuestion);
        const searchContext = chunks
          .filter(c => c.content.trim().length > 20) // Filter noisy/tiny chunks
